...
recipient=''
submitter_id=''
while (($#)); do case $1 in --dry-run|--confirm) mode=$1;; --report-recipient) shift; recipient=${1:-};; --submitter-id) shift; submitter_id=${1:-};; *) usage; exit 2;; esac; shift; done
case "${action}" in
 list-redacted) python3 - "${registry}" <<'PY'
import json,sys
for x in json.load(open(sys.argv[1])): print({'submitterId':x['submitterId'],'tokenHash':x['tokenHash'][:12]+'…','revoked':x['revoked']})
PY
 ;;
 add|revoke) [[ -n ${mode} && -f ${registry} && -f ${access} ]] || { usage; exit 2; }
   [[ ${mode} == --dry-run ]] && { printf 'would %s submitter registry and access map\n' "${action}"; exit 0; }
   exec 9>"${registry}.lock"; flock -x 9
