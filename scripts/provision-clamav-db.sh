...
[[ -n ${mode} && -n ${url} && -n ${expected_sha256} && -n ${version} ]] || {
	usage
	exit 2
}
[[ ${url} == https://* ]] || {
	printf '%s\n' 'database URL must use HTTPS' >&2
	exit 2
}
[[ ${expected_sha256} =~ ^[0-9a-f]{64}$ ]] || {
	printf '%s\n' 'SHA-256 must be lowercase hexadecimal' >&2
	exit 2
}
[[ ${database} =~ ^(main|daily|bytecode)$ ]] || {
	printf '%s\n' 'database must be main, daily, or bytecode' >&2
	exit 2
}
...
for command in curl docker sha256sum; do command -v "${command}" >/dev/null 2>&1 || {
...
[[ ${actual_sha256} == "${expected_sha256}" ]] || {
	printf '%s\n' 'ClamAV database digest mismatch; nothing was published' >&2
	exit 1
}
