...
while (($#)); do case $1 in --message) shift; message=${1:-};; --ip) shift; ip=${1:-};; --helo) shift; helo=${1:-};; --from) shift; from=${1:-};; --queue-id) shift; queue=${1:-};; --detached) :;; *) usage; exit 2;; esac; shift; done
[[ -r ${message} ]] || { usage; exit 2; }
