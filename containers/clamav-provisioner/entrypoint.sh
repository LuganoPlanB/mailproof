...
provision_artifact() {
  [[ ${url} == https://* ]] || { printf '%s\n' 'artifact mode requires MAILPROOF_CLAMAV_DATABASE_URL using HTTPS' >&2; return 2; }
  [[ ${expected_sha256} =~ ^[0-9a-f]{64}$ ]] || { printf '%s\n' 'artifact mode requires lowercase MAILPROOF_CLAMAV_DATABASE_SHA256' >&2; return 2; }
  [[ -n ${version} && ${database} =~ ^(main|daily|bytecode)$ ]] || { printf '%s\n' 'artifact mode requires version and a valid database name' >&2; return 2; }
  ...
  [[ ${actual} == "${expected_sha256}" ]] || { printf '%s\n' 'ClamAV artifact digest mismatch; refusing publication' >&2; return 1; }
...
case ${1:-once} in
  once)
    ...
    ;;
  update-loop)
    [[ ${mode} == latest ]] || { printf '%s\n' 'clamav updater requires MAILPROOF_CLAMAV_PROVISION=latest' >&2; exit 2; }
    interval=${MAILPROOF_CLAMAV_UPDATE_INTERVAL_SECONDS:-21600}
    [[ ${interval} =~ ^[1-9][0-9]*$ ]] || { printf '%s\n' 'update interval must be a positive integer' >&2; exit 2; }
    while true; do
      set +e
      provision_latest
      update_status=$?
    ...
    ;;
  *) printf '%s\n' 'usage: mailproof-clamav-provisioner {once|update-loop}' >&2; exit 2 ;;
esac
